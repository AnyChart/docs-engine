...
function searchFor(query) {
     if (query) {
         ...
         $.post("/" + version + "/search-data", {"q": query}, function(res) {
             $("#page-content").html('<div class="row"><div class="col-lg-17"><button type="button" class="btn btn-default btn-blue visible-xs"> <i class="glyphicon glyphicon-arrow-left"></i> Back</button><h1 class="search"><button type="button" class="btn btn-default btn-blue hidden-xs"> <i class="glyphicon glyphicon-arrow-left"></i> Back</button>Search results for <span>' + query + '</span> </div></div></h1>');
             if (res.length) {
                 $(res).each(function() {
